...
func seedIfNeeded() async {
    ...
    context.insert(contentsOf: [costco, sunoco, bp, dmv, insurer, car])
    ...
    let reg = RegistrationExpense(amount: 75, date: Date(timeIntervalSinceNow: -3600*24*90), mileage: 69616, stateOrProvince: "OH", validFrom: Date(timeIntervalSinceNow: -3600*24*100), validTo: Calendar.current.date(byAdding: .year, value: 1, to: Date())!, merchant: dmv, car: car)

    let policy = InsurancePolicy(policyNumber: "POL-12345", merchant: insurer)
    policy.cars.append(car)
    context.insert(policy)

    let premium = InsuranceExpense(amount: 450, date: Date(timeIntervalSinceNow: -3600*24*30), mileage: 69000, coverageType: "Full", deductible: 500, termStart: Date(timeIntervalSinceNow: -3600*24*30), termEnd: Calendar.current.date(byAdding: .year, value: 1, to: Date())!, paidInFull: true, policy: policy, merchant: insurer, car: car)
    ...
    context.insert(contentsOf: [f1, f2, f3, reg, premium, maint])
    try? context.save()
}
